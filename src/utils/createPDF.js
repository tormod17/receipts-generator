  ...
  return rowData.map((guest, index) => 
    fields.map( field  => {
      if ((/datum/).test(field)) {
        ...
        return  (/date/).test(foramattedDate) ? '-': foramattedDate;
      } else if ( field === '#') {
...
export function createPDF(client) {
  ...
  const headers = titleHeaders.map(title => {
    return {
      colSpan: title === getText('REASON') ? 2 : 1,
      text: title,
      style: 'tableHeader'
    };
  });
  ...
  const newGuestRows = guestRows.map(row => {
    ...
    const secondRow =   [ 
        ...
        '',
        '',
        { text: getText('TRANS.LETTER.CLEANING'), fillColor: '#CCCCCC', colSpan: 2},
        '',
        {
          text: row[5],
          alignment: 'right',
    ...
    const thirdRow =[
       ...
       '',
       '',
      { text: getText('TRANS.SRV.FEE'), colSpan: 2},
      '',
      {
        text: row[6],
        alignment: 'right',
  ...
  let totalFields = [ 
...

    {
      ...
      value: calculateTaxTotals(getText("INVS"), guests, corrections) + '€'
    }
  ]; 
  ...
  const totalRows  = totalFields.map(obj =>
        [
          ...
          {
            text: obj.name,
            bold: (isInvoice && obj.name === getText('TOTAL.INV')) || (!isInvoice && obj.name === getText('TOTAL.PAYOUT')),
            colSpan: 2
          },
          '',
          { 
  ...
  const dd = {
    ...
    content: [{
      ...
      {
        text: getText('LETTER.TITLE') + ' | ' + getText('LETTER.ADDRESS.STREET') + ' | ' + getText('LETTER.ADDRESS.CITY'),
        width: 100,
        style: 'detailText'
      },
      {
        columns: [
          {
            text: client[getText('TRANS.CUSTOMER')] + '\n'+ client[getText('TRANS.STREET')] + '\n'+ client[getText('TRANS.POSTCODE')] + ' '+ client[getText('TRANS.TOWN')] + '\n'+ client[getText('TRANS.CUSTOMER.NO')],
            alignment: 'left',
            style: 'address'
          },
          {
            text: getText('LETTER.TITLE') + '\n '+ getText('LETTER.ADDRESS.STREET') + '\n'+  getText('LETTER.ADDRESS.CITY') +' \n ' +getText('LETTER.EMAIL'),
            alignment: 'right',
            style: 'address'
          }
        ]
      ...
      {
        text: isInvoice ? getText('LETTER.HEADING1') : getText('LETTER.HEADING2'),
        style: 'heading'
      },
      {
        text: getText('LETTER.TEXT6') + formatDate(new Date(Number(client[getText('INV.DATE')])),  'LL', 'de'),
        alignment: 'right'
      },
      {
        text: getText('INV.NUMBER') + client[getText('INV.NUMBER')],
        style: 'invoiceNumber'
      },
      {
        text: getText('LETTER.SUBHEADING'), 
        style: 'subHeading'
      },
      {
        style: 'tableExample',
      ...
      {
        text: isInvoice ? getText('LETTER.TEXT') : getText('LETTER.TEXT1'),
        style: 'salu',
        margin: [0 , 10, 0 , 10]
      },
      {
        text: getText('LETTER.SALU'),
        style: 'salu'
      },
      {
      ...
      {
        text: getText('LETTER.TEXT2'),
        style: 'finfo'
      },
      {
        text: getText('LETTER.TEXT3'),
        style: 'finfo'
      },
      {
        text: getText('LETTER.TEXT4'),
        style: 'finfo'
      },
      {
        text: getText('LETTER.TEXT5'),
        style: 'finfo'
      }
    ],
    styles: {
